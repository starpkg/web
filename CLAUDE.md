...
- **`static.go`** — static-file serving. `static_dir(...)` builds a read-only
  ...
  GET/sendfile). Path safety = `path.Clean` + dotfile/`@eaDir` rejection
  (`.well-known` carve-out) + lexical `withinRoot` + `EvalSymlinks` real-path
  check (`openWithin`, blocks symlink escape). Mount registry is copy-on-write
  so the serve path reads it lock-free. Tests in `static_test.go` (source-shaped).
